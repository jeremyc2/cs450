...
vowelMatrix <- function(gamma){
  invisible(mapply(runVowel, g = gamma , c = 2^(seq(-15,5,2))))
}

letterMatrix <- function(gamma){
  invisible(mapply(runLetters, g = gamma , c = 2^(seq(-15,5,2))))
}

mapply(vowelMatrix, g = 2^(seq(-5,15,2)))
mapply(letterMatrix, g = 2^(seq(-5,15,2)))